...
```javascript
jdbc:postgresql://localhost:5432/lt_test?options=-c%20search_path=\"$user\",public,oracle,lt_catalog%20-c%20statement_timeout=90000
```
这种方式必须提供完整的search_path列表，不会自动包含public schema。其效果和ltsql中执行set search_path一致。

### currentSchema方式,jdbc提供了currentschema参数用于设置当前schema
```javascript
